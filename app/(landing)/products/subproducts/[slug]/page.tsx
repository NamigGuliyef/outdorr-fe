...
export async function generateStaticParams() {
  const subproducts = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/subproducts`,
  {
    next: { revalidate: 60 },
  }).then((res) => res.json())
...
export default async function ProductDetails({
  ...
  return (
    <Section className="mt-24 lg:mt-[88px]">
      ...
      <Section className="container my-12 md:my-20 grid grid-cols-1 gap-6 px-6 md:grid-cols-6 lg:grid-cols-12 lg:px-0">
        <div className="font-sf text-darkgray md:col-span-4 lg:col-span-4">
          <h1 className="md:mb-8 text-2xl font-semibold sm:text-3.2xl md:text-5xl">
            Features
          </h1>
        ...
        >
          {featuresIds.map((feature: ISubProductFeature, idx) => (
            <div
              key={idx}
              className="flex gap-4 md:gap-6 rounded-2xl md:rounded-[30px] border border-[#BABABA] p-8"
            >
