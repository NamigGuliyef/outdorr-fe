...
import "swiper/css/effect-fade";
  ...
  export async function generateStaticParams() {
    const projects = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/projects`,
    {
      next: { revalidate: 60 },
    }).then((res) => res.json())
          ...
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="rounded-[30px] md:w-1/2 p-6 lg:p-8 lg:min-h-[248px] h-min flex flex-col gap-6 text-darkgray border border-[#BABABA]">
              ...
              <div
                ...
                dangerouslySetInnerHTML={{ __html: featuresId[0]?.description || "" }}
                ></div>
            </div>
          </div>
          <div className="flex flex-col md:flex-row gap-6">
            <div className="rounded-[30px] order-2 md:order-1 md:w-1/2 p-6 lg:p-14 flex flex-col justify-between text-darkgray min-h-[540px] md:min-h-[444px] border border-[#BABABA]">
              <div className="flex flex-col gap-6">
                ...
                <h1 className="text-3.2xl leading-[120%] md:text-5xl capitalize text-darkgray font-semibold">
                  {usedProductsId?.title} used in this Project
                </h1>
                <p className="text-2xl leading-[120%]">{usedProductsId?.description}</p>
              </div>
              <Button
                to="#"
